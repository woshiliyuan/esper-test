...
public class SimulateClientConnection extends Thread {
    ...
    public void run() {
        ...
        final TradeEvent[] market = new TradeEvent[Symbols.SYMBOLS.length];
        ...
        try {
            ...
            do {
                ...
                for (int i = 0; i < eventPer10Millis; i++) {
                    ...
                    final TradeEvent theEvent = market[tickerIndex++];
                    //note the cloning here, although we don't change volume or price
                    final TradeEvent simulatedEvent = (TradeEvent) theEvent.clone();
                    if (executor == null) {
