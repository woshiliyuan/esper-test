...
public class CEPProvider {
	...
	public static ICEPProvider getCEPProvider() {
		String className = System.getProperty("esper.limit.provider",
				EsperCEPProvider.class.getName());
		try {
	...
	public static class EsperCEPProvider implements ICEPProvider {
		...
		// only one of those 2 will be attached to statement depending on the
		// -mode selected
		private UpdateListener updateListener;
		private Object subscriber;

		public static int sleepListenerMillis;
		...
		public void init(final int _sleepListenerMillis) {
			...
			try {
				Class configurationHAClass = Class
						.forName("com.espertech.esperha.client.ConfigurationHA");
				configuration = (Configuration) configurationHAClass
						.newInstance();
				System.out
						.println("=== EsperHA is available, using ConfigurationHA ===");
			} catch (ClassNotFoundException e) {
				configuration = new Configuration();
			} catch (Throwable t) {
				System.err
						.println("Could not properly determine if EsperHA is available, default to Esper");
				t.printStackTrace();
				configuration = new Configuration();
			}
			configuration.addEventType("Market", TradeEvent.class);
			configuration.addEventType("TradeEvent", TradeEvent.class);

			// EsperJMX enablement - if available
			try {
				Class.forName("com.espertech.esper.jmx.client.EsperJMXPlugin");
				configuration.addPluginLoader("EsperJMX",
						"com.espertech.esper.jmx.client.EsperJMXPlugin", null);// will
																				// use
																				// platform
																				// mbean
																				// -
																				...
																				// line
				System.out
						.println("=== EsperJMX is available, using platform mbean ===");
			} catch (ClassNotFoundException e) {
				;
			}
			...
			subscriber = new TradeEventSubscriber();
			epRuntime = epService.getEPRuntime();
		}

		public void registerStatement(String statement, String statementID) {
			EPStatement stmt = epAdministrator
					.createEPL(statement, statementID);
			if (System.getProperty("esper.limit.ul") != null) {
	...
	// }
	//
	// public void update(long count, double sum) {
